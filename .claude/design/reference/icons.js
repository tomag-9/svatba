// icons.js — Icon component using real Lucide path data (inlined so it renders
// reliably in every capture path, not CSS-mask dependent). Documented
// substitution: the source repo defines no icon system of its own (see
// readme "Iconography") — Lucide is the closest-fit open CDN icon set.
window.ICON_PATHS = {
...
window.Icon = function Icon({ name, size = 20, color = 'currentColor', style = {} }) {
  const inner = window.ICON_PATHS[name] || window.ICON_PATHS['circle-dashed'];
...
window.categoryIcon = function categoryIcon(category) {
  return window.CATEGORY_ICON[category] || 'circle-dashed';
};

window.NAV_ITEMS = [
  { id: 'dashboard', label: 'Prehľad', icon: 'layout-dashboard' },
  { id: 'timeline', label: 'Os', icon: 'clock' },
  { id: 'tasks', label: 'Úlohy', icon: 'square-check' },
  { id: 'guests', label: 'Hostia', icon: 'users' },
  { id: 'finance', label: 'Rozpočet', icon: 'wallet' },
];
